...
var Hermite = {
	init: function (workerPath) {
		...
		if (!workerPath)
			throw new Error('Fail to provide worker\'s path when initializing.');
	...
	resize: function (obj, callback) {
		...
		worker.onmessage = function (event) {
			...
			if (obj.output === 'canvas') {
				...
				if (sourceElement[1] === 'IMG')
					return callback(self._canvasToImage(canvas, obj.mimeType, obj.quality));
				return callback(canvas);
			} else {
	...
	_extract: function (source) {
		...
		if (['CANVAS', 'IMG'].indexOf(results[1]) === -1)
			throw new Error('#resize() expects element type of `img` or `canvas`.');
	...
	_mimeConverter: function (format) {
		...
		if (index === -1)
			throw new Error('#inputImage() outputType can only be `raw`, `png`, `jpg` or `gif`');
