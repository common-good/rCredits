...
s_a[239]="Alabama|Alaska|APO|Arizona|Arkansas|California|Colorado|Connecticut|Delaware|District of Columbia|Florida|Georgia|Hawaii|Idaho|Illinois|Indiana|Iowa|Kansas|Kentucky|Louisiana|Maine|Maryland|Massachusetts|Michigan|Minnesota|Mississippi|Missouri|Montana|Nebraska|Nevada|New Hampshire|New Jersey|New Mexico|New York|North Carolina|North Dakota|Ohio|Oklahoma|Oregon|Pennsylvania|Rhode Island|South Carolina|South Dakota|Tennessee|Texas|Utah|Vermont|Virginia|Washington|West Virginia|Wisconsin|Wyoming";
...
function print_country(country_id, dft_country, dft_state){
	...
	option_str.length=0; // zap any previous list items
	var x, i=0;
	for(x in country_arr) {
	  option_str.options[i++] = new Option(country_arr[x],country_arr[x]);
	  if (dft_country == country_arr[x]) option_str.selectedIndex = i-1;
	}
	if (dft_country == "") {
	  option_str.selectedIndex = 238; // 238 is US
	  dft_state = "Massachusetts";
	}
	print_state('edit-state', option_str.selectedIndex, dft_state);
}

function print_state(state_id, country_index, dft_state){
	var option_str = document.getElementById(state_id);
	option_str.length=0; // zap any previous list items
	var x, i=0; country_index++;
	var state_arr = s_a[country_index].split("|");
	for(x in state_arr) {
	  option_str.options[i++] = new Option(state_arr[x],state_arr[x]);
	  if (dft_state == state_arr[x]) option_str.selectedIndex = i-1;
	}
}
