...
class html2text
{
    ...
    var $search = array(
	  ...
	  "/<br[^>]*>\n/i",				 // <br> at end of line (don't add a space) - cp 20061218
        "/[\n\t]+/",                             // Newlines and tabs
        '/<script[^>]*>.*?<\/script>/i',         // <script>s -- which strip_tags supposedly has problems with
        '/<style[^>]*>.*?<\/style>/i',           // <style>s - cp 20061218
        '/<x>.*?<\/x>/i',				 // Things to eliminate for sure - cp 20061218
        '/<!--x(.*?)x-->/i',                     // Things that should only appear in the plain text - cp 20061218
        '/<h[123][^>]*>(.+?)<\/h[123]>/ie',      // H1 - H3
        '/<h[456][^>]*>(.+?)<\/h[456]>/ie',      // H4 - H6
        '/<p[^>]*>/i',                           // <P>
        '/<br[^>]*>/i',                          // <br>
//        '/<b[^>]*>(.+?)<\/b>/ie',                // <b>
        '/<b[^>]*>(.+?)<\/b>/i',                // <b> - cp 20081015 CAPS looks good, but gets tagged as spam
        '/<i[^>]*>(.+?)<\/i>/i',                 // <i>
        '/<u[^>]*>(.+?)<\/u>/i',                 // <u> - cp 20061218
        '/<ul[^>]*>/i',	                         // <ul> - cp 20071011
        '/<ol[^>]*>/i',                          // <ol> - cp 20071011
        '/<li[^>]*>/i',                          // <li>
        '/(<\/ul>|<\/ol>|<\/li>)/i',             // </ul>, </ol> and </li> - cp 20071011
//        '/<a href="([^"]+)"[^>]*>(.+?)<\/a>/ie', // <a href="">
        '/<hr[^>]*>/i',                          // <hr>
        ...
        '/<td[^>]*>(.+?)<\/td>/i',               // <td> and </td>
        '/<th[^>]*>(.+?)<\/th>/i',               // <th> and </th>
        '/<head>.*?<\/head>/i',			 // <head>...</head> - cp 20081015
        '/&nbsp;/i',
        '/&quot;/i',
        '/&gt;/i',
        '/&lt;/i',
    ...
    var $replace = array(
	  ...
	  '<br>',						// <br> at end of line (don't add a space) - cp 20061218
        ' ',                                    // Newlines and tabs
        '',                                     // <script>s -- which strip_tags supposedly has problems with
        '',							// <style>s - cp 20061218
        '',							// Things to eliminate for sure - cp 20061218
        '\\1',                                  // Things that should only appear in the plain text - cp 20061218
        "strtoupper(\"\n\n\\1\n\n\")",          // H1 - H3
        "ucwords(\"\n\n\\1\n\n\")",             // H4 - H6
        "\n\n",                                 // <P> - cp 20061218
        "\n",                                   // <br>
//        'strtoupper("\\1")',                    // <b>
        ...
        "\n\n",                                 // <ol> - cp 20071011
        "  * ",                                  // <li>
        "\n",                                   // </ul>, </ol> and </li> - cp 20071011
//        '$this->_build_link_list($link_count++, "\\1", "\\2")', - cp 20061218
                                                // <a href="">
    ...
    function _convert()
    {
        ...
        if ( $this->width > 0 ) {
        	$text = wordwrap($text, $this->width, "\n", true); // cp 20081024 cut long words
        }
