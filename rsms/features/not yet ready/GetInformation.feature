...
# Always round the demand to an integer. Round the demand down to three significant digits and use "million" once it passes a million.
  ...
  And phone %number1 balance is $100
  And phone %number1 unavailable is $7.50
  And the demand for rCredits is $26,987.56
  When phone %number1 says "information"
  Then we say to phone %number1 "balance and demand"
  | @balance | @unavailable | @demand |
  | $100 | $7.50 | $26,987.56 |
  Then show phone %number1's account balance and the demand for rCredits: "Your balance is $100, including $7.50 not yet available. The current demand for rCredits is $26,900"

Scenario: Getting information, larger amounts
  Given phone %number1 is a player
  And phone %number1's balance is $100,000.01
  And phone %number1's unavailable is $99,908.00
  And the demand for rCredits is $1,226,987.25
  When phone %number1 says "information"
  Then show phone %number1's account balance and the demand for rCredits: "Your balance is $100,000.01, including $99,908 not yet available. The current demand for rCredits is $1.22 million"
  | @balance | @unavailable | @demand |
