...
//MODEL: <input id='slider1' class='carpe-slider orientation-horizontal target-display_slider size-200 position-100 stops-10' />
...
// This script requires carpe_common.js v0.2
var CARPE = CARPE || {};
CARPE.Sliders = {
	version: '2.0 Beta',
	elements: [], // Array that holds all slider objects in a page. // unused 20100321 CP

	// Method 'init': Loops through slider elements and creates the corresponding slider objects. 
	init: function() {
		var nm;
		CARPE.Sliders.elements = CARPE.getElementsByClass(CARPE.Slider.prototype.sliderClassName);
		for (var i in CARPE.Sliders.elements) { // Loops through slider elements.
			nm = CARPE.Slider.prototype.panelClassName + CARPE.Slider.prototype.nameValueSeparator + CARPE.Sliders.elements[i].id;
			if (!document.getElementById(nm)) { // If the slider is not yet present in the page.
				cgsliders[cgsliders.length] = new CARPE.Slider(CARPE.Sliders.elements[i]); // Create the new slider object. // 20100321 CP
			}
		}
		cgReactText(''); // CP
		return;
...
CARPE.Slider = function(elmnt, params, location, style) {
...
//		this.valueElmnt.value = this.value.toFixed(1) + '%'; // Set the value of the hidden input slider display. // 20100319 CP
		...
		if (this.target && !preventTargetUpdate) {
//			this.updateTarget();
		}
		return this;
	},
	...
	update: function() {
		return this.setValue(this.value || this.target.value, true); // cp 20100429
//		return this.setValue(this.value || this.valueElmnt.value, true);
	},

	// Class method 'cgReact': maintain sum of all sliders equal 100 (start with an "other" option set to 100?
	/* cgReact: function() {
		var totalvalue;
		var otherpreftotal;
		var that;
		...
		if(lastsliderchanged != this.id) { // changing a different slider
//testpr('last='+lastsliderchanged + ' .. THISID='+this.id);
			lastsliderchanged = this.id;
			for (var i in cgsliders) valueprefs[cgsliders[i].id] = cgsliders[i].value; // remember the current value preferences
		}				
		totalvalue = 0;
		otherpreftotal = 0;
		oldtopvalue = 0;

		for (var i in cgsliders) { // get new total value and total of other preferences and see if there's a new max value
			that = cgsliders[i];
			totalvalue += that.value; 
			if(that.id != this.id) otherpreftotal += valueprefs[that.id]; 
			if(that.value > oldtopvalue) oldtopvalue = that.value; // get max
		}
// if(this.id == that.id) testpr('iID='+that.id + ' .. ivalue=' + that.value + ' .. thisvalue='+this.value+' tot='+totalvalue);

		newmaxvalue = Math.min(maxvalue99, Math.round(Math.max(oldtopvalue, maxvalue99-totalvalue) * idealmaxratio));
// testpr('newmax='+newmaxvalue+' oldmax='+maxvalue+' otherpreftotal='+otherpreftotal);
//		if(maintaintop) if(newmaxvalue > maxvalue) if(newmaxvalue < maxvalue99) maxvalue = newmaxvalue; // adjust scale to match highest

	  if(otherpreftotal == 0) {
//			maintaintop = false; // NOT USED
		} else {
//		} else if((totalvalue > maxvalue) || ((totalvalue < maxvalue) && maintaintop && totalvalue > 0)) { // keep the total right
//			if(!maintaintop) 
			maxvalue = newmaxvalue; // set new scale once the total reaches the normal limit
//			maintaintop = true;
			for (var i in cgsliders) { // adjust the others in proportion to preferences
				that = cgsliders[i];
				reacting = true;
if(that.id != this.id) testpr('setting slider#'+that.id+' to '+(valueprefs[that.id] * (maxvalue99 - this.value) / otherpreftotal));
				if(that.id == this.id) {
					this.setValue(this.value, true); // reset to current value, just to update slider display with new maxvalue
				} else that.setValue(valueprefs[that.id] * (maxvalue99 - this.value) / otherpreftotal, false);
				reacting = false;
			}
		} // else if(totalvalue == 0) maintaintop = false;
	},
*/
	...
	function cgReactText(si) {  // si only supplied when pct is changed directly
		var that;
		var newvalue = si ? parseInt(cgsliders[si].target.value.replace('%','')) : 0;
		var totalvalue = -newvalue; // omit this pct from the total
		var dpy;

		for (var i in cgsliders) if(cgsliders[i].active) totalvalue += cgsliders[i].value; // get new total value
//pr('newvalue='+newvalue+' totalvalue='+totalvalue);
		for (var i in cgsliders) { // set displays to the right percentage
			that = cgsliders[i];
			if(that.active && (i != si)) { // don't change inactive sliders or newvalued slider (if any)
				dpy = totalvalue == 0 ? 0 : that.value * (100 - newvalue) / totalvalue;
//pr('thatvalue='+that.value+' dpy='+dpy);
	//testpr('id='+that.id+' value='+that.value+' tot='+totalvalue+' dpy='+dpy);
				that.valueElmnt.value = dpy.toFixed(1) + '%'; // Set the value of the hidden input slider display. // 20100319 CP
...
//if (CARPE.addLoadEvent) CARPE.addLoadEvent(CARPE.Sliders.init);
